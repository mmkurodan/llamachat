...
public final class CalendarPromptFactory {
    ...
    public static String buildJudgePrompt(String userInput, String nowIso8601) {
        return "あなたは Google Calendar 操作判定専用のアシスタントです。\n"
                ...
                + "- title に「〜の予定」「〜のイベント」「〜の日程」「〜のスケジュール」などの語を付け足してはいけない\n"
                + "- title にはユーザ入力に含まれる予定名をそのまま抜き出して入れる\n"
                + "- 予定名が曖昧な場合は title を null にする\n"
                + "- UPDATE / DELETE では title は既存イベント検索キーになるため、1文字でも変形してはいけない\n"
                + "- 文中の「予定」「イベント」「日程」「スケジュール」は予定名そのものではない場合があるので、title に含めてはいけない\n"
                + "- 「◯◯の予定」「◯◯という予定」「◯◯イベント」などの表現でも、予定名は「◯◯」だけにする\n"
                + "- 助詞「の」の後ろに「予定」「イベント」「日程」「スケジュール」が続く場合、それらは予定名ではない\n"
                + "- title に「予定」「イベント」「日程」「スケジュール」という単語が残っていたら誤りとして修正する\n"
                ...
                + "- 「11時から2時間」「16時から2時間」などは絶対時間であり、現在時刻とは無関係に解釈する\n"
                + "- 「11時から2時間」は必ず 11:00〜13:00 とする\n"
                + "- 「本日の11時から2時間」は必ず今日の日付の 11:00〜13:00 とする\n"
                + "- 「今から2時間」「2時間後」などの表現だけを相対時間として扱う\n"
                + "- 絶対時間と相対時間が混在する場合は絶対時間を優先する\n"
                + "- ISO8601 の「+09:00」はタイムゾーンであり、時間に加算してはいけない\n"
                + "- たとえば「2026-05-23T11:00:00+09:00」は 11 時であり、20 時として扱ってはいけない\n"
                + "- タイムゾーンは計算対象ではなく、ISO8601 にそのまま付与するだけにする\n"
